...
app.use((req, res, next) => { logger.debug(`Request body size: ${req.socket.bytesRead}`); next(); });
...
async function startup() {
  try {
    ...
    const server = app.listen(SERVER_PORT, SERVER_HOST, () => {
      ...
      fs.appendFileSync('/tmp/metric-service-process-pids', `${process.pid}\n`);
      logger.notice(`Metric-Service PID: ${process.pid}`);
